...
export default function MediaDownloaderPage() {
  ...
  const validateUrl = (input: string) => {
    try {
      ...
      // Also check for common media signatures in params if extension is missing
  ...
  const handleDownload = async () => {
    ...
    if (!validateUrl(url)) {
      setStatus('error');
      setErrorMessage("Protocol Mismatch: The URL must be a direct link to a media file or binary asset (e.g. ending in .mp4, .mp3).");
      return;
    }
    ...
    try {
      ...
      if (err.message.includes('Failed to fetch') || err.name === 'TypeError') {
        setErrorMessage("CORS Protocol Block: The server hosting this file prevents direct browser-side extraction for security. Try the 'Direct Open' fallback below.");
      } else {
    ...
    <div className="container mx-auto px-6 py-12 md:py-20">
      <div className="mb-12 animate-reveal">
        ...
        <h1 className="text-3xl md:text-5xl font-headline font-black text-foreground uppercase tracking-tight">
          Universal <span className="text-primary italic">Media Downloader</span>
        </h1>
        <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
          Professional browser-side file extraction for all formats. Retrieve video and audio assets directly from URLs with high-fidelity binary preservation.
        </p>
      </div>
          ...
          <Card className="glass-card border-border shadow-2xl overflow-hidden relative group">
            ...
            <CardContent className="pt-10 space-y-8">
              <div className="space-y-4">
                <Label className="text-[10px] font-black text-foreground/50 uppercase tracking-[0.2em] ml-1">Direct Media URL</Label>
                ...
                <div className="p-4 rounded-xl bg-primary/5 border border-primary/10 flex items-center gap-3">
                   ...
                   <p className="text-[9px] text-foreground/40 font-bold uppercase tracking-widest leading-relaxed">
                     Supports MP4, MP3, WAV, WebM, MOV, MKV, PDF, ZIP and more.
                   </p>
                </div>
              </div>
          ...
          <Card className="glass-card border-border shadow-xl overflow-hidden group">
            <CardHeader className="py-6 border-b border-border bg-primary/5 group-hover:bg-primary/10 transition-colors">
              <CardTitle className="text-[10px] font-black uppercase tracking-[0.3em] text-primary flex items-center gap-2">
                <Zap className="w-4 h-4" /> Extraction Standards
              </CardTitle>
            </CardHeader>
            <CardContent className="p-8 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-[11px] font-medium text-foreground/50 uppercase tracking-wider leading-relaxed">
                <div className="flex gap-4">
                  ...
                  <p><span className="text-foreground font-black">All Formats:</span> Protocol accepts any valid binary media stream URL.</p>
                </div>
                <div className="flex gap-4">
                  <div className="w-1.5 h-1.5 rounded-full bg-primary mt-1.5 shrink-0" />
                  <p><span className="text-foreground font-black">Private:</span> Asset is converted to a Blob locally. Zero server logging.</p>
                </div>
                <div className="flex gap-4">
                  <div className="w-1.5 h-1.5 rounded-full bg-primary mt-1.5 shrink-0" />
                  <p><span className="text-foreground font-black">DRM Shield:</span> Does not extract from encrypted social platforms (YT/IG).</p>
                </div>
                <div className="flex gap-4">
                  <div className="w-1.5 h-1.5 rounded-full bg-primary mt-1.5 shrink-0" />
                  <p><span className="text-foreground font-black">Direct File:</span> Link must resolve to a file, not a landing page.</p>
                </div>
              </div>
            </CardContent>
          </Card>
              ...
              {status === 'success' && (
                <div className="space-y-6 animate-in zoom-in duration-500">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-[10px] text-foreground/40 font-medium uppercase">Binary pushed to device storage.</p>
                  </div>
                  <div className="w-full p-4 rounded-xl bg-secondary border border-border flex items-center gap-4">
                     ...
                     <div className="text-left overflow-hidden">
                        ...
                        <p className="text-[8px] font-bold text-primary uppercase tracking-widest">Verified Payload</p>
                     </div>
                  </div>
          ...
          <div className="p-6 rounded-[2.5rem] bg-primary/5 border border-primary/10 flex items-start gap-5">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[11px] text-foreground/40 leading-relaxed font-medium">
                Browsers enforce Cross-Origin Resource Sharing. If a remote host blocks direct browser requests, the studio provides a "Direct Open" protocol to manually save the asset.
              </p>
            </div>
          </div>
        </div>
